...
from utils.utils import get_dataset, get_dataloader, compute_accuracy_and_loss
...
def main():
    ...
    _, base_model_loss = compute_accuracy_and_loss(base_model,test_dl,DEVICE)
    ...
    for method,color in zip(methods, colors):
        df_acc_method = pd.read_csv(f'../../output_bert/toxigen/{method}_finetuning/loss.csv')
        df_acc_method["std"] = df_acc_method["std"]/2
        ...
        plt.plot([0]+df_acc_method["k"].to_list(), np.hstack([base_model_loss,df_acc_method["mean"]]), color = color, label = method)
    ...
    plt.tight_layout()
    plt.savefig(f'../vis/vis_bert_toxigen/total_loss.png')


    data_groups = pd.DataFrame(columns=["method","k","acc","std"])

    for method in methods:
        df_acc_method_groups = pd.read_csv(f'../../output_bert/toxigen/{method}_finetuning/loss_by_groups.csv')
        df_acc_method_groups = df_acc_method_groups.loc[df_acc_method_groups["k"].isin(ks)]
        df_acc_method_groups["method"] = method
        df_acc_method_groups["std"] = df_acc_method_groups["std"]/2
    ...
    for group in data_groups["group"].unique():
        ...
        _,base_model_loss_group = compute_accuracy_and_loss(base_model,test_dl_group,DEVICE)
        ...
        for method,color in zip(methods, colors):
            ...
            plt.plot([0]+df_group_method["k"].to_list(), np.hstack([base_model_loss_group,df_group_method["mean"]]), color = color, label = method)

        plt.xlabel('K Removed Training Samples')
        plt.ylabel('Loss')
        plt.title(f'Group Loss: {group}')
        plt.xticks(ks)
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'../vis/vis_bert_toxigen/{group}_loss.png')
